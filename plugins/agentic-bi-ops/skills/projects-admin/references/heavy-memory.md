...
- **Never vendor, fork, or copy Basic Memory's source into this repo, and never
  modify+redistribute it** - that would force `agentic-bi-ops` itself to become AGPL.
- The user installs it from its official source; our plugin only talks to it over MCP (a
  separate process = mere aggregation, no license inheritance). `agentic-bi-ops` stays MIT.
