...
`scripts/Handoff-SessionStartHook.ps1` makes a saved handoff **surface itself** the moment a
new or resumed session starts, so you do not have to remember `Board-Handoff.ps1 -Resume`. It
is **opt-in** - a hook that runs on every session start should be a deliberate choice.

## What it does

On session start it reads the hook's stdin JSON (`source`, `cwd`), looks for a local
`HANDOFF.md` at the current repo root, and - if present - emits a one-line `additionalContext`
so the assistant immediately knows there is a handoff to resume:
