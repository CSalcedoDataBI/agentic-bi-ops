<#  Resolve-SkillOwner.ps1 — where does a skill's failure feedback belong?
    ...
    project you happen to be working in. This resolves the routing:
      ...
      - plugin  'agentic-bi-ops'  -> file to CSalcedoDataBI/agentic-bi-ops (this tool's board)
    ...
    EXAMPLE
      Resolve-SkillOwner -Scope plugin -Plugin agentic-bi-ops
      Resolve-SkillOwner -Scope project -CurrentRepo CSalcedoDataBI/agentic-bi-ops
#>
[CmdletBinding()]
param(
    ...
    [string]$Plugin,
    [string]$CurrentRepo,
    [string]$ToolRepo = 'CSalcedoDataBI/agentic-bi-ops'
)

switch ($Scope) {
    'plugin' {
        if ($Plugin -eq 'agentic-bi-ops') {
